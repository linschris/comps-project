...
if [ "$1" -eq 0 ]; then
	while read line
		do
			...
			$YTDL -f best "http://www.youtube.com/watch?v=$line" -o ./videos/"${name}"/"%(id)s.%(ext)s"
			# $YTDL --write-thumbnail --skip-download "http://www.youtube.com/watch?v=$line" -o ./thumbnails/"%(id)s.%(ext)s"
	...
	while read line
		do
			if [ "$limit" -gt 0 ]; then
				limit=$(($limit-1))
				$YTDL -f best "http://www.youtube.com/watch?v=$line" -o ./videos/"${name}"/"%(id)s.%(ext)s" 2>&1 | tee log.txt
				# $YTDL "--write-thumbnail --skip-download" "http://www.youtube.com/watch?v=$line" -o ./thumbnails/"%(id)s.%(ext)s" 2>&1 | tee log.txt
